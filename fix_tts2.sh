...
INNER_EOF
awk '/if \(audioBytes != null\) \{/ { system("cat temp.kt"); skip=1 } /\} else \{/ { if(skip) skip=0 } skip { next } { if(!skip && !/if \(audioBytes != null\)/ && !/\} else \{/ && !/resumeChatbotMode\(\)/) print }' app/src/main/java/com/example/MainActivity.kt > temp2.kt
# Wait, AWK is too complicated for this multiline replace. Let's just use Python!
